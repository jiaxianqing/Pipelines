...
outfile3 <- paste(outfile,".QTLs.sites_info.csv", sep="")
...
getQTLTable(SNPset = df_filt, method = "QTLseq", alpha = 0.01, export = TRUE, fileName = outfile1)
getQTLTable(SNPset = df_filt, method = "Gprime", alpha = 0.01, export = TRUE, fileName = outfile2)
write.table(df_filt[], file = outfile3, sep = "\t",quote = F)
