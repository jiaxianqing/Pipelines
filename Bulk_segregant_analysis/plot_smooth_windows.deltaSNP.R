...
ggplot(df.flt) +
  geom_rect(data = centrome, aes(xmin=start/1000000, xmax=end/1000000, ymax=max(df$Stats), ymin=-max(df$Stats)), fill="grey")+
  geom_line(aes(x = (Start + End)/2/1000000, y = Stats), size = 0.5, colour="black") + 
  geom_line(aes(x = (Start + End)/2/1000000, y = abs(CI95)), size = 0.5, colour="#990000") + 
  geom_line(aes(x = (Start + End)/2/1000000, y = -abs(CI95)), size = 0.5, colour="#990000") + 
  geom_line(aes(x = (Start + End)/2/1000000, y = abs(CI99)), size = 0.5, colour="darkblue") + 
  geom_line(aes(x = (Start + End)/2/1000000, y = -abs(CI99)), size = 0.5, colour="darkblue") + 
  ...
  labs(x="Chromosome lLength (Mbp)",
       y=ylabname,
       title=titlename) +
  #geom_hline(aes(yintercept=8.08), colour="#990000", linetype="dashed", size = 0.5) + 
